...
function page<TIn, TOut>(
  input: PaginatedResponse<TIn>,
  map: (item: TIn) => TOut,
...
type ApiInventorySummary = InventorySummary & {
  productID?: number;
  sKU?: string;
};
...
export const usersApi = {
  list: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiUser>>(`/Users${queryString(params)}`).then(
      (r) => page(r, mapUser),
    ),
...
export const customersApi = {
  list: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiCustomer>>(
      `/Customers${queryString(params)}`,
    ).then((r) => page(r, mapCustomer)),
  getById: (id: number) =>
    request<ApiCustomer>(`/Customers/${id}`).then(mapCustomer),
  ...
  updateStatus: (id: number, status: Customer["status"]) =>
    request<ApiCustomer>(`/Customers/${id}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }).then(mapCustomer),
  getOrders: (id: number) =>
    request<ApiOrder[]>(`/Customers/${id}/orders`).then((items) =>
      items.map(mapOrder),
    ),
};
...
export const suppliersApi = {
  list: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiSupplier>>(
      `/Suppliers${queryString(params)}`,
    ).then((r) => page(r, mapSupplier)),
  getById: (id: number) =>
    request<ApiSupplier>(`/Suppliers/${id}`).then(mapSupplier),
  ...
  updateStatus: (id: number, status: Supplier["status"]) =>
    request<ApiSupplier>(`/Suppliers/${id}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }).then(mapSupplier),
  getProducts: (id: number) =>
    request<ApiProduct[]>(`/Suppliers/${id}/products`).then((items) =>
...
export const categoriesApi = {
  list: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiCategory>>(
      `/Categories${queryString(params)}`,
    ).then((r) => page(r, mapCategory)),
  listAll: () =>
    request<ApiCategory[]>("/Categories/all").then((items) =>
      items.map(mapCategory),
    ),
  getById: (id: number) =>
    request<ApiCategory>(`/Categories/${id}`).then(mapCategory),
  ...
  updateStatus: (id: number, status: Category["status"]) =>
    request<ApiCategory>(`/Categories/${id}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }).then(mapCategory),
  getProducts: (id: number) =>
    request<ApiProduct[]>(`/Categories/${id}/products`).then((items) =>
      items.map(mapProduct),
    ),
};
...
export const productsApi = {
  list: (params: QueryParams = {}) =>
    ...
    ).then((r) => page(r, mapProduct)),
  listAll: () =>
    request<ApiProduct[]>("/Products/all").then((items) =>
      items.map(mapProduct),
    ),
  getById: (id: number) =>
    request<ApiProduct>(`/Products/${id}`).then(mapProduct),
  ...
  updateStatus: (id: number, status: Product["status"]) =>
    request<ApiProduct>(`/Products/${id}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }).then(mapProduct),
};
...
export const ordersApi = {
  list: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiOrder>>(`/Orders${queryString(params)}`).then(
      (r) => page(r, mapOrder),
    ),
  ...
  update: (id: number, data: Partial<Order>) =>
    request<ApiOrder>(`/Orders/${id}`, {
      method: "PUT",
      body: JSON.stringify({ ...data, status: apiOrderStatus(data.status) }),
    }).then(mapOrder),
  confirm: (id: number) =>
    request<ApiOrder>(`/Orders/${id}/confirm`, { method: "POST" }).then(
      mapOrder,
    ),
  cancel: (id: number) =>
    request<ApiOrder>(`/Orders/${id}/cancel`, { method: "POST" }).then(
      mapOrder,
    ),
};
...
export const inventoryApi = {
  list: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiInventorySummary>>(
      `/Inventory${queryString(params)}`,
    ).then((r) => page(r, mapInventorySummary)),
  getTransactions: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiInventoryTransaction>>(
      `/Inventory/transactions${queryString(params)}`,
    ).then((r) => page(r, mapInventoryTransaction)),
...
export const dashboardApi = {
  getKpis: (fromDate?: string, toDate?: string) => {
    ...
    return request<KpiData>(`/Dashboard/kpis${query ? `?${query}` : ""}`);
  },

  getCharts: (fromDate?: string, toDate?: string) => {
    const params = new URLSearchParams();
    ...
    return request<DashboardCharts>(
      `/Dashboard/charts${query ? `?${query}` : ""}`,
    );
  },
};

export const reportsApi = {
  salesReport: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiOrder>>(
      `/Reports/sales${queryString(params)}`,
    ).then((r) => page(r, mapOrder)),
  productsReport: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiProduct>>(
      `/Reports/products${queryString(params)}`,
    ).then((r) => page(r, mapProduct)),
  inventoryReport: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiInventorySummary>>(
      `/Reports/inventory${queryString(params)}`,
    ).then((r) => page(r, mapInventorySummary)),
  customersReport: (params: QueryParams = {}) =>
    request<PaginatedResponse<ApiCustomer>>(
      `/Reports/customers${queryString(params)}`,
    ).then((r) => page(r, mapCustomer)),
};
