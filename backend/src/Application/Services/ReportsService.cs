...
public class ReportsService : IReportsService
{
    ...
    public async Task<PaginatedResponse<OrderDto>> GetSalesReportAsync(
        QueryParams qp,
        int? customerId = null,
        string? status = null,
    ...
    public async Task<PaginatedResponse<ProductDto>> GetProductsReportAsync(
        QueryParams qp,
        int? categoryId = null)
    {
    ...
    public async Task<PaginatedResponse<InventorySummaryDto>> GetInventoryReportAsync(
        QueryParams qp,
        string? stockStatus = null)
    {
        var summaries = await _inventoryRepo.GetInventorySummaryAsync(
            qp.Search,
            stockStatus,
            qp.Page,
            qp.PageSize);

        var total = await _inventoryRepo.GetInventorySummaryCountAsync(
            qp.Search,
            stockStatus);
        ...
        return new PaginatedResponse<InventorySummaryDto>
        {
            Data = dtos,
            Total = total,
            Page = qp.Page,
    ...
    public async Task<PaginatedResponse<CustomerDto>> GetCustomersReportAsync(
        QueryParams qp)
    {
        ...
        foreach (var c in customers)
        {
            ...
            dto.OrderCount = await _context.Orders
                .CountAsync(o =>
                    o.CustomerId == c.Id &&
                    o.Status != "cancelled");

            dto.TotalPurchases = await _context.Orders
                .Where(o =>
                    o.CustomerId == c.Id &&
                    o.Status != "draft" &&
                    o.Status != "cancelled")
